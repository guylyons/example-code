(function ($, Drupal) {
    ...
    Drupal.behaviors.une_emergency = {
    ...
    function UNEEmergency() {
        ...
        var refreshDuration = 30000, cookieExpireDays = 1;
        ...
        var processTier = function(tierObject, cookieTimestamps) {
            var delta = {newItems:false,
                removedItems:false};

            if (typeof tierObject !== "undefined") { // tier exists
                if (tierObject.items.length > 0) { // tier is nonempty
                    var feedTimestamps = [];
                    ...
                    var timestampsUniqueToFeed = new Set(feedTimestamps.filter((a) => !cookieTimestamps.has(a)));
                    feedTimestamps = new Set(feedTimestamps);
                    var timestampsUniqueToCookies = new Set(Array.from(cookieTimestamps).filter((a) => !feedTimestamps.has(a)));
                ...
                } else if (cookieTimestamps.size > 0) { // feed is empty, but cookies are nonempty
        ...
        var processAll = function(jo) {
            ...
            if (tier1Changes.removedItems) {
                for (let key in storedEvents["tier1"]) { // "key" here is the timestamp, value is cookie name
            ...
            if (tier2Changes.removedItems) {
                for (let key in storedEvents["tier2"]) { // "key" here is the timestamp, value is cookie name
            ...
            for (let ii=0; ii<jo.tier1.items.length; ii++) {
                setCookie("UNEEMERGENCY1." + ii.toString(), jo.tier1.items[ii].timestamp);
            }
            for (let ii=0; ii<jo.tier2.items.length; ii++) {
                setCookie("UNEEMERGENCY2." + ii.toString(), jo.tier2.items[ii].timestamp);
            }

            // we are on /emergency
            if (regEmergencyPage.test(location.pathname)) {
                // items removed from either feed
                var $content = $("#block-system-main .content");
                ...
                if (esDisplayContainer.length == 0) {
                    $content.first().prepend($("<div>").text("<!-- -->").attr("id", "esdisplay-container"));
                ...
                if (jo.tier1.items.length > 0) { // we have a tier-1 emergency
                ...
                } else if (jo.tier2.items.length > 0) { // having no tier-1 emergency, we have a tier-2 emergency
                    // Generate the tier2 display
                    esDisplayContainer.addClass(currentAlert);
                    esDisplayContainer.html(generateItemsHtml(jo.tier2.items, "2"));
                } else if (esDisplayContainer.length > 0 && esDisplayContainer.hasClass(currentAlert)) { // having no emergencies, display the all-clear text
                    esDisplayContainer.removeClass(currentAlert);
                    esDisplayContainer.html(generateAllClearHtml()); // add the all-clear text here
                    esDisplayContainer.css({"border": "1px solid gainsboro",
                        "margin-bottom": "20px",
                        "padding": "20px"});
                }

                timeoutIteration = setTimeout(function(){ fetch(); }, refreshDuration);
            } else if (!regAdminPage.test(location.pathname)) { // we are not on /emergency or /admin
                ...
                if (tier1Changes.newItems) {
                    ...
                    if ($emergencyHeaderContainer.length == 0 && (jo.tier1.items.length > 0 || jo.tier2.items.length > 0)) {
                        $("body .wrapper")
                            .prepend($("<div/>")
                    ...
                    if (jo.tier1.items.length > 0) { // fill banner with tier-1 items
                        $emergencyHeaderContainer.html(generateItemsBannerHtml(jo.tier1.items[0]));
                        timeoutIteration = setTimeout(function(){ fetch(); }, refreshDuration);
                    } else if (jo.tier2.items.length > 0) { // fill banner with tier-2 items
                        $emergencyHeaderContainer.html(generateItemsBannerHtml(jo.tier2.items[0]));
                        timeoutIteration = setTimeout(function(){ fetch(); }, refreshDuration);
                ...
                if (regEmergencyPage.test(location.pathname)) {
                    if ($("p.esdisplay-currdate").length == 0) {
                        var $dateLine = $("<p/>")
                            ...
                            .html("Today is " + getWeekText(dt.getDay()) + ", " + getMonthText(dt.getMonth()) + " " + dt.getDate() + ", " + dt.getFullYear());
        ...
        var generateItemsBannerHtml = function(item) {
            ...
            var emergencyTitle = item.title + " <span class='emergency-date clearfix'>Posted: " + item.dow + " " + item.propperDate + " " + item.propperTime + " " + item.timezone + "</span>";

            // Create the div object to return
            var $emDiv = $("<div/>").attr("id", "emergency-display").append($("<a/>").attr("href", "/emergency").html(emergencyTitle));
        ...
        var generateItemsHtml = function(items, tier) {
            ...
            // Set the date string
            var cDateString = getWeekText(dt.getDay()) + ", " + getMonthText(dt.getMonth()) + " " + dt.getDate() + ", " + dt.getFullYear();

            // Append the h3 alert text
            $htmlContainer.append($("<h3/>").text("Alert!"));

            // Append the date string
            $htmlContainer.append($("<div/>").attr("id", "esdisplay-currdate").text("Today is " + cDateString));
            ...
            for (let ii = 0; ii < items.length; ii++) {
                ...
                var $eventLeftContainer = $("<div/>").addClass("es-t" + tier + "-event-left");

                // Add the DOW if the date of the event is not today
                if (items[ii].propperDate !== cDateString) {
                    $eventLeftContainer.append($("<div/>").addClass("event-dow emergency-date").text("Posted: " + items[ii].dow));
                }

                // Add the event date and time to the left container
                $eventLeftContainer
                    .append($("<div/>").addClass("event-date emergency-date").text(items[ii].propperDate))
                    .append($("<div/>").addClass("event-time emergency-date").text(items[ii].propperTime + " " + items[ii].timezone));
                ...
                var $eventRightContainer = $("<div/>")
                    ...
                    .append($("<div/>").addClass("event-title").text(items[ii].title))
                    .append($("<div/>").addClass("event-description").text(items[ii].description));
        ...
        var generateAllClearHtml = function() {
            ...
            $htmlContainer.append($("<p/>").text("There is no emergency at the University of New England. The campus is operating under normal conditions."));

            //
            $htmlContainer
                .append($("<p/>").text("In the event of an emergency, this site will be updated with information about the nature of the incident, how to remain safe and the point at which safety has been restored ")
                    .append($("<span/>").text("after the event").css({"color": "rgb(39, 39, 39)",
        ...
        var getAllCookies = function() {
            ...
            for (let ii=0; ii<keyValuePairs.length; ii++) {
                ...
                var thisTier = cookieKey.startsWith("UNEEMERGENCY1") ? "tier1" : "tier2";
        ...
        var setCookie = function(cKey, cVal) {
            ...
            cVal = escape(cVal) + ((cookieExpireDays == null) ? "" : ";expires=" + exdate.toUTCString() + "; path=/;");
